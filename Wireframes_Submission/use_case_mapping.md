...
| `login_register.html` | **UC-001 (Registration and Login processing)** | Allows users to create a Buyer or Agent account, or login to an existing one. |
| `search_properties.html` | **UC-002 (Search and Filter properties)** | Main interface for searching cities/ZIPs and applying filters (Price, Beds, Baths). Features an interactive map placeholder. |
| `property_details.html` | **UC-003 (View Property Details)** | The comprehensive detail view of a listing, showing image galleries, specs, facts, and agent contact info. |
...
| `agent_profile_edit.html` | **US-006 (Register & Manage Agent Profile)** | UI allowing an approved agent to update their bio, contact information, profile photo, and upload license documents. |
...
| `create_listing.html` | **US-007 (Create Property Listing)** | Detailed form for agents to enter property details, upload media, set the status, and submit a new listing for review. |
| `edit_listing.html` | **US-008 (Edit / Delete Property Listing)** | Form to modify pricing, description, media, or change the status of an existing property listing. |
...
| `admin_approve_agents.html` | **US-012 (Post-condition: Mod Approval)** | The interface where admins review submitted agent license documents and approve/reject their applications. |
